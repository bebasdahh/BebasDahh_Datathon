...
SEARCH_FILTER = "logo"
TARGET_PAGES = 1000
OUTPUT_FILENAME = f'dataset_logo_pdki_{YEAR_FILTER}_{STATUS_FILTER}.csv'
LOGO_DIRECTORY = f'logos_{YEAR_FILTER}'
...
async def scrape_search_results(url, target_page_number):
    ...
    if not os.path.exists(LOGO_DIRECTORY):
        ...
        print(f"Created directory: '{LOGO_DIRECTORY}/'")
...
async def apply_filters(page):
    ...
    await page.fill("input >> nth=0", SEARCH_FILTER)
...
def parse_page_data(soup):
    ...
    for container in brand_containers:
        ...
        if img_tag and 'src' in img_tag.attrs:
            img_src = img_tag['src']
            if img_src.startswith('data:image') and app_number:
                try:
                    header, encoded = img_src.split(',', 1)
                    file_ext = header.split(';')[0].split('/')[1]
                    filename = f"{app_number}.{file_ext}"
                    filepath = os.path.join(LOGO_DIRECTORY, filename)
                    ...
                    print(f"⚠️  Could not save image for {app_number}: {e}")

        brand_info['Logo File'] = logo_path
